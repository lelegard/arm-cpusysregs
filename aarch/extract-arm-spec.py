...
def expand(tarball, reffile=None, force=False):
    ...
    with tarfile.open(tarball) as tar:
        # Check if all files alread start with the dirname.
        start_all = True
...
with open(FEATURES_MD_OUT, 'w') as output:
    ...
    print(file=output)
    print('| %-*s | %-*s | %-*s | %-*s | %s' % (name_width, headers.name, opt_width, headers.optional, mand_width, headers.mandatory, sys_width, headers.sysregs, headers.description), file=output)
    print('| %s | %s | %s | :%s: | %s' % (name_width * '-', opt_width * '-', mand_width * '-', (sys_width - 2) * '-', desc_width * '-'), file=output)
    for key in sorted(all_features.keys(), key=str.lower):
        feat = all_features[key]
        print('| %-*s | %-*s | %-*s | %s | %s' % (name_width, feat.name, opt_width, feat.optional, mand_width, feat.mandatory, feat.sysregs.center(sys_width), feat.description), file=output)
...
for filename in xmlfiles:
    ...
    if regname is not None and not re.match('.*<.*>.*', regname):
        ...
        # Get fieldset if we need to generate code for it.
        xfields = xmltree.find('.//fields')
        if reg is not None and reg.cppfield is not None and xfields is not None:
            reg.bitfields = []
